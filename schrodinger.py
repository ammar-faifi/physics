...
class Schrodinger(object):
    ...
    def _set_psi_x(self, psi_x):
        self.psi_mod_x = (
            psi_x * np.exp(-1j * self.k[0] * self.x) * self.dx / np.sqrt(2 * np.pi)
        )

    def _get_psi_x(self):
        return (
            self.psi_mod_x
            * np.exp(1j * self.k[0] * self.x)
            * np.sqrt(2 * np.pi)
            / self.dx
        )
    ...
    def _set_dt(self, dt):
        if dt != self.dt_:
            ...
            self.k_evolve = np.exp(
                -0.5 * 1j * self.hbar / self.m * (self.k * self.k) * dt
            )
...
def gauss_x(x, a, x0, k0):
    ...
    """
    return (a * np.sqrt(np.pi)) ** (-0.5) * np.exp(
        -0.5 * ((x - x0) * 1.0 / a) ** 2 + 1j * x * k0
    )
...
ymax = V0
ax1 = fig.add_subplot(
    211, xlim=xlim, ylim=(ymin - 0.2 * (ymax - ymin), ymax + 0.2 * (ymax - ymin))
)
(psi_x_line,) = ax1.plot([], [], c="r", label=r"$|\psi(x)|$")
(V_x_line,) = ax1.plot([], [], c="k", label=r"$V(x)$")
center_line = ax1.axvline(0, c="k", ls=":", label=r"$x_0 + v_0t$")

# title = ax1.text(1, 0.95, "")
title = ax1.text(
    0.1,
    0.9,
    "",
    bbox={"facecolor": "w", "alpha": 0.3, "pad": 5},
...
ax2 = fig.add_subplot(
    212, xlim=klim, ylim=(ymin - 0.2 * (ymax - ymin), ymax + 0.2 * (ymax - ymin))
)
...
anim = animation.FuncAnimation(
    fig, animate, init_func=init, frames=frames, interval=30, blit=True
)
...
# anim.save(
#     "/Users/ammaralfaifi/Downloads/schrodinger_barrier.mp4",
#     fps=30,
#     dpi=300,
